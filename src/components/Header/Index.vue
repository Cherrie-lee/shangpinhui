...
methods:{
    ...
    goSearch(){
        // 路由传递参数：
        // 第一种：字符串形式
        // this.$router.push("/search/"+ this.inputCon+"?k="+this.inputCon.toUpperCase())
        // 第二种：模板字符串
        // this.$router.push(`/search/${this.inputCon}?k=${this.inputCon.toUpperCase()}`)
        // 第三种：对象写法
        /* this.$router.push({
            name:"search",
            params:{
                inputCon:this.inputCon || undefined
            },                                  
        },()=>{},()=>{}) */
        // params参数可以传递也可以不传递，但是如果传递是空串，如何解决？
        // this.$router.push({name:"search",params:{inputCon:'' | undefined},query:{k:this.inputCon.toUpperCase()
        // }
        // })
    ...
    async logout(){
        // 退出登录：1.需要发请求：通知服务器退出登录【清楚一些数据：token】
